...
def parse(f):
    ...
    for i in range(header["functionCount"]):
        ...
        if (functionHeader["flags"] >> 5) & 1:
            ...
            saved_pos = f.tell()
            large_offset = (functionHeader["functionName"] << 24 )  | functionHeader["offset"]
            f.seek(large_offset)
    ...
    # Segment 7: LiteralValueBuffer (was ArrayBuffer in HBC96)
    ...
    # Segment 10: ObjShapeTable (HBC98) - objShapeTableCount entries of 8 bytes each
    objShapeTableS[2] = header["objShapeTableCount"] * 8
...
def export(obj, f):
    ...
    objShapeTableS[2] = header["objShapeTableCount"] * 8
    write(f, objShapeTable, objShapeTableS)
    ...
    for overflowedFunctionHeader in overflowedFunctionHeaders:
        ...
        large_offset = (smallFunctionHeader["functionName"] << 24 )  | smallFunctionHeader["offset"]
        f.seek(large_offset)
        for key in functionHeaderS:
